...
export async function toggleArticleLikeAction(articleId: number): Promise<TResAction<string | null>> {
   try {
      const result = await api.post<TRes<string>>(`${ENDPOINT.ARTICLE_LIKE_TOGGLE}/${articleId}`);
      const { data } = result;
      return { status: "success", message: result.message, data: data };
   } catch (error: any) {
      return { status: "error", message: error?.message, data: null };
