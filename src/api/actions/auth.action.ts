...
import { clearTokens } from "@/helpers/cookies.helper";
...
export async function getInfoAction(): Promise<TResAction<{ user: TUser; isTotp: boolean } | null>> {
    try {
        const result = await api.get<TRes<{ user: TUser; isTotp: boolean }>>(ENDPOINT.AUTH.GET_INFO);
        const { data } = result;
        return { status: "success", message: result.message, data: data };
    } catch (error: any) {
        return { status: "error", message: error?.message, data: null };
